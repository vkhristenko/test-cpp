...
struct Connection : public std::enable_shared_from_this<Connection> {
    explicit Connection(StreamSocket s) : s_{std::move(s)} {}
...
private:
    void DoRead() noexcept {
        TCPP_PRETTY_FUNCTION();
        s_.AsyncRead(buffer_.data(), buffer_.size(),
                     [self = shared_from_this(),
                      this](Result<std::size_t, std::string> re_bytes) {
                         if (re_bytes.HasError()) {
                             ::fmt::print("failed to read!");
                             return;
                         }

                         ::fmt::print("read in {} bytes",
                                      re_bytes.ValueUnsafe());
    ...
    void DoWrite(std::size_t const nbytes) noexcept {
        ...
        s_.AsyncWrite(buffer_.data(), nbytes,
                      [self = shared_from_this(),
                       this](Result<std::size_t, std::string> re_nbytes) {
                          if (re_nbytes.HasError()) {
                              ::fmt::print("failed to write!");
                              return;
                          }

                          ::fmt::print("written out {} bytes",
                                       re_nbytes.ValueUnsafe());
                          DoRead();
                      });
    }
...
private:
    void DoAccept() noexcept {
        acceptor_.AsyncAccept([this](Result<StreamSocket, std::string> s) {
            ...
            auto conn =
                std::make_shared<Connection>(std::move(s).ValueUnsafe());
            conn->Start();
        });
    }
