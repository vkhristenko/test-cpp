...
private:
    ...
    constexpr static auto kAuxSize = sizeof(core::SmallFunction<void()>) - 
        core::SmallFunction<void()>::GetMaxObjectSize();
    constexpr static auto kMaxObjectSize = 2 * sizeof(core::SmallFunction<void()>) - kAuxSize;
...
/// allows the usage of forwrad decl which is not possible if doing type aliasing
